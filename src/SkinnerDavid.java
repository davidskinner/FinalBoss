...
public class SkinnerDavid implements IAgent
{
	...
	{
//		if(model.getSpriteCountOpponent() > 0)
//		{
//			for (int i = 0; i < model.getSpriteCountOpponent(); i++)
//			{
//				enemyArrayList.add(new Enemy(model.getX(i),model.getY(i),i));
//			}
//		}
...
//			this.yPosition = y;
	...
	public static class Sprite
	{
		...
		public void updateEnemyMembers()
		{
//			for (Enemy e : enemyArrayList)
//			{
//				e.prevX = e.xPosition;
//				e.prevY = e.yPosition;
//				e.xPosition = model.getXOpponent(e.index);
//				e.yPosition = model.getYOpponent(e.index);
//				double distance = Math.sqrt( Math.pow(e.xPosition - e.prevX, 2) + Math.pow(e.yPosition - e.prevY, 2));
//				double time = (double)(e.currentTime - e.prevTime);
//				e.velocity = distance/time;
//				e.predx = e.xPosition + (e.velocity* time)/100;
//			}
		}
		...
		public void update()
		{
			...
			//calculate enemy velocites here to shoot where enemy will be
			//updates the
		...
		private Enemy getEnemyNearestMe()
		{
			...
			for (int i = 0; i < model.getSpriteCountOpponent(); i++)
			{
				...
				//gets the distance to the opponenet
		...
		{
			State[][] costMap = TreeUtility.uniformCostSearch(terrainMap, terrainMap[(int) x() / 10][(int) y() / 10], terrainMap[(int) x / 10][(int) y / 10]);
		...
		{
			//Center the shot on where the enemy will be todo
			if (enemy.distance <= Model.MAX_THROW_RADIUS * Model.MAX_THROW_RADIUS)
			{
				...
				shoot((float)enemyArrayList.get(enemy.index).predx, (float)enemyArrayList.get(enemy.index).predy);
		...
		{
			//assigning the previousx values
			for (int i = 0; i < sprites.length; i++)
			{
				sprites[i].prevX = sprites[i].x();
				sprites[i].prevY = sprites[i].y();
				sprites[i].prevTime = currentTime;
			...
			if (index == 0)
			{
				...
				System.out.println("prevx: " + String.valueOf(x()) + " prevy: " + String.valueOf(y()) + " prevTime: " + String.valueOf(prevTime));
				System.out.println("nextx: " + String.valueOf(x) + " nexty: " + String.valueOf(y) + " currentTime: " + String.valueOf(currentTime));
				//sqrt((x2 -x1)^2 + (y2 -y1)^2)
				double distance = Math.sqrt( Math.pow(xPosition - prevX, 2) + Math.pow(yPosition - prevY, 2));
				double time = (double)(currentTime - prevTime);
				double velocity = distance/time;
				System.out.println("the time: " + String.valueOf(time));
				System.out.println("the velocity: " + String.valueOf(velocity));
				System.out.println();
				System.out.println("currentx = " + String.valueOf(x()));
				System.out.println("currenty = " + String.valueOf(y()));
				double predx =x() + (velocity*time)/100;
				double predy = y() + (velocity*time)/100;
				System.out.println("predictedx = " + String.valueOf(predx));
				System.out.println("predictedy = " + String.valueOf(predy));
